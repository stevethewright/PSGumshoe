<#
.SYNOPSIS
    Get WMI Temporary Event Start events (EventId 5860).
.DESCRIPTION
    Get WMI Temporary Event Start events (EventId 5860). from a local or remote host. Events can be filtered by fields.
.INPUTS
    System.IO.FileInfo
.OUTPUTS
...
#>
function Get-EventWmiTemporaryEvent {
    ...
    param (
        ...
        # Query associated with the temporary subscription.
        [Parameter(Mandatory = $false,
                   ValueFromPipelineByPropertyName = $true)]
        [string[]]
        $Query,

        # NameSpace against the query is being performed against.
        [Parameter(Mandatory = $false,
                   ValueFromPipelineByPropertyName = $true)]
        [string[]]
        $NameSpace,
    ...
    process {
        Search-EventLogUsertData -EventId 5860 -Provider "Microsoft-Windows-WMI-Activity" -ReturnRecord -SubElement -ParamHash $MyInvocation.BoundParameters | ForEach-Object {
            [xml]$evtxml = $_.toxml()
            $ProcInfo = [ordered]@{}
            ...
            $ProcInfo['ClientMachine'] = $evtxml.Event.UserData.Operation_TemporaryEssStarted.ClientMachine
            $ProcInfo['Namespace'] = $evtxml.Event.UserData.Operation_TemporaryEssStarted.NamespaceName
            $ProcInfo['Query'] = $evtxml.Event.UserData.Operation_TemporaryEssStarted.Query
            $ProcInfo['ProcessId'] = $evtxml.Event.UserData.Operation_TemporaryEssStarted.ProcessId
            $ProcInfo['PossibleCause'] = $evtxml.Event.UserData.Operation_TemporaryEssStarted.PossibleCause
