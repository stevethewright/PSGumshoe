...
function Get-SysmonFileBlockShredding {
    ...
    param (
        ...
        # Process GUID for the process that  was blocked from shredding the file.
        [Parameter(Mandatory = $false,
                   ValueFromPipelineByPropertyName = $true)]
        [string[]]
        $ProcessGuid,

        # Process Id for the process that was blocked from shredding the file.
        [Parameter(Mandatory = $false,
                   ValueFromPipelineByPropertyName = $true)]
        [string[]]
        $ProcessId,

        # Image path for the process that was blocked from shredding the file.
        [Parameter(Mandatory = $false,
                   ValueFromPipelineByPropertyName = $true)]
        [string[]]
        $Image,

        # Name of the file that an attempt was done against.
        [Parameter(Mandatory = $false,
                   ValueFromPipelineByPropertyName = $true)]
        [string[]]
        $TargetFileName,
    ...
    process {
        Search-SysmonEvent -EventId 28 -ParamHash $MyInvocation.BoundParameters
