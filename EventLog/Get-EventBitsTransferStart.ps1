...
function Get-EventBitsTransferStart {
    ...
    begin {
        ...
        if ($Params.Keys -contains "JobId") {
            $Params.Remove("JobId") | Out-Null
            $Params.Add('Id', $JobId) | Out-Null
        }

        if ($Params.Keys -contains "JobTitle") {
            $Params.Remove("JobTitle") | Out-Null
            $Params.Add('name', $JobTitle) | Out-Null
        }
    }

    process {
        Search-EventLogEventData -EventId 59 -ParamHash $Params -Provider "Microsoft-Windows-Bits-Client" -RecordType "BitsTransferComplete"
