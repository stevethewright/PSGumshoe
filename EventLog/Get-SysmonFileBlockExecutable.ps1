...
function Get-SysmonFileBlockExecutable {
    ...
    param (
        ...
        # Process GUID for the process that  was blocked from creating the file.
        [Parameter(Mandatory = $false,
                   ValueFromPipelineByPropertyName = $true)]
        [string[]]
        $ProcessGuid,

        # Process Id for the process that was blocked from creating the file.
        [Parameter(Mandatory = $false,
                   ValueFromPipelineByPropertyName = $true)]
        [string[]]
        $ProcessId,

        # Image path for the process that was blocked from creating the file.
        [Parameter(Mandatory = $false,
                   ValueFromPipelineByPropertyName = $true)]
        [string[]]
        $Image,
    ...
    process {
        Search-SysmonEvent -EventId 27 -ParamHash $MyInvocation.BoundParameters
